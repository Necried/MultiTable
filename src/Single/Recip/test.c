...
void test(int exp, int fromtable, int tableend, __uint64_t *reciptable,
  ...
  // define variant
  int id, maxid, minid;
  __uint64_t ncorrnd, omaxncorrnd, ominncorrnd, ontests, ntests, oontests,
  ...
  for (int j = 0; j <= exp; j++) {
    ...
    __uint64_t omaxiulperr = omaxytest.i - omaxytrue.i;
    printf("%3d %2d %5d %5s %10.2e %08lX %10.2e %016lX %10.2e %016lX %10.2e "
           "%10.2e %6.2f\n",
           maxid, 0, 0, "     ", // FIXME
           mantissa_to_double(omaxx), (__uint64_t)omaxx, omaxytest.d,
           omaxytest.ull, omaxytrue.d, omaxytrue.ull, omaxulperr,
           (double)omaxiulperr, 100.0 * omaxncorrnd / ntests);

    printf("Min \n");

    __uint64_t ominiulperr = ominytest.i - ominytrue.i;
    printf("%3d %2d %5d %5s %10.2e %08lX %10.2e %016lX %10.2e %016lX %10.2e "
           "%10.2e %6.2f\n",
           minid, 0, 0, "     ", // FIXME
           mantissa_to_double(ominx), (__uint64_t)ominx, ominytest.d,
